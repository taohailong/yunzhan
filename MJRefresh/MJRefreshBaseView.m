    ...
    switch (state) {
		...
		case MJRefreshStateRefreshing:
        {
            ...
            if ([self.beginRefreshingTaget respondsToSelector:self.beginRefreshingAction]) {
                ...
                [self sendNewObjcMsg:self.beginRefreshingTaget _sel:self.beginRefreshingAction withObj:self];
...
-(void) sendNewObjcMsg:(id) _target _sel:(SEL) _sel withObj:(id) _obj
{
    void (*objc_msgSendTyped)(id self, SEL _cmd, id _ddservice) = (void*)objc_msgSend;
    
    objc_msgSendTyped(_target, _sel, _obj);
}
